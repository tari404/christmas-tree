...
router.get('/wx-wysq-user-info-base',function (req,res) {
	...
	var keyis = getParam(search_str,'keyis'); console.log(keyis);
...
router.post('/getjscfg',function (req,res) {
	var tmp = fs.readFileSync( path.join(__dirname, 'wechat_data/jsapi_ticket.js') );
	tmp = tmp.toString();
	tmp = JSON.parse(tmp);// console.log(tmp);

	var jsapi_ticket = tmp['dinyuehao'].ticket;
...
router.get('/getact',function (req,res) {
	var tmp = fs.readFileSync( path.join(__dirname, 'wechat_data/access_token.js') );
	tmp = tmp.toString();
	tmp = JSON.parse(tmp);
...
function refreshAct() {
	// 充电帮订阅号
	var appid = 'wx7cbb86fca076aff1'; 
	var appsecret = '2a5537d12b57e9b1c954fd2fb2b2ea18';
	...
	// 充电帮服务号
	appid = 'wx442ea0e9f7102adf';
	appsecret =  'ce795419487e25a478306944418d3161';
	opts_se = {
		host : host,  
        port : '443',  
        path : '/cgi-bin/token?grant_type=client_credential&appid='+appid+'&secret='+appsecret,  
	...
	setTimeout(function () {
		...
		fs.writeFileSync(path.join(__dirname, 'wechat_data/access_token.js'), data);
	}, 1000*10);  
}
...
function refreshJst() {
	...
	var act_din = tmp['dinyuehao'].access_token;
	...
	var opts_din = {
		host : host,  
        port : '443',  
        path : '/cgi-bin/ticket/getticket?access_token='+act_din+'&type=jsapi',
        method : 'GET'  
	};
	var opts_fu = {
		host : host,  
	...
	setTimeout(function () {
		...
		fs.writeFileSync(path.join(__dirname, 'wechat_data/jsapi_ticket.js'), ticks);
	}, 1000*10);
}
